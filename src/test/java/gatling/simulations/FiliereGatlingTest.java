 ...
 */
public class FiliereGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all filieres").get("/services/sgrcore/api/filieres").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new filiere")
                        .post("/services/sgrcore/api/filieres")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"nomFiliere\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_filiere_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created filiere").get("/services/sgrcore${new_filiere_url}").headers(headersHttpAuthenticated)).pause(10)
                )
                .exec(http("Delete created filiere").delete("/services/sgrcore${new_filiere_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Filiere entity").exec(scn);
