...
public class ReclamationGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all reclamations")
                    .get("/services/sgrcore/api/reclamations")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new reclamation")
                        .post("/services/sgrcore/api/reclamations")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"nomReclamant\": \"SAMPLE_TEXT\"" +
                                ", \"prenomReclamant\": \"SAMPLE_TEXT\"" +
                                ", \"niveauEtude\": \"SAMPLE_TEXT\"" +
                                ", \"objetReclamation\": \"SAMPLE_TEXT\"" +
                                ", \"motifReclamation\": \"SAMPLE_TEXT\"" +
                                ", \"justificatif\": \"SAMPLE_TEXT\"" +
                                ", \"etatReclamation\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_reclamation_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created reclamation").get("/services/sgrcore${new_reclamation_url}").headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created reclamation").delete("/services/sgrcore${new_reclamation_url}").headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Reclamation entity").exec(scn);
