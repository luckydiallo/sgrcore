 ...
 */
public class MatiereGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all matieres").get("/services/sgrcore/api/matieres").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new matiere")
                        .post("/services/sgrcore/api/matieres")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"nomMatiere\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_matiere_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created matiere").get("/services/sgrcore${new_matiere_url}").headers(headersHttpAuthenticated)).pause(10)
                )
                .exec(http("Delete created matiere").delete("/services/sgrcore${new_matiere_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Matiere entity").exec(scn);
